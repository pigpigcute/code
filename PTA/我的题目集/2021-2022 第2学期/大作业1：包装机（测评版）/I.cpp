...
#include <stdbool.h>
#include <string.h>
struct ANode //包含用于记录答案的数组和该数组元素个数的结构佄1�7
{
    char data[1000010]; //记录答案
    int back;           //记录答案数量
};
typedef struct ANode *ans_vector;
ans_vector CreateAns_Vector() //在堆内存申请答案记录结构体空闄1�7
{
...
int main()
{
    ...
    stack s = CreateStack(limit);       //建立栄1�7
    ...
    while (1)
    {
        ...
        else
        {
            op--; //轨道序号-1==对应下标
            if (shelve_pos[op] == m)
                continue;                              //若当前轨道的头部指针以指向下标m，则说明当前轨道上的扢�有元素已全部弹出，无操作
            push(s, shelve[op][shelve_pos[op]++], av); //否则将当前头部元素压入栈丄1�7
        }
    }
    for (int i = 0; i < (av->back); i++) //将答案数组中扢�有元素输凄1�7
    {
        printf("%c", av->data[i]);
    }
